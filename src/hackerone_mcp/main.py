...
class MDCComplianceChecker:
    ...
    def _extract_security_context(self) -> Dict[str, Any]:
        ...
        return {
            'triage_questions': h1_rules.get('report_triage', {}).get('classification_questions', []),
            'severity_rules': h1_rules.get('report_triage', {}).get('severity_assessment', {}),
    ...
    def validate_severity_assignment(self, severity: str, report_data: Dict) -> Dict[str, Any]:
        """Validate if severity assignment follows MDC rules"""
        severity_rules = self.security_context['severity_rules'].get(severity.lower(), {})
        ...
        if 'ask_before_marking' in severity_rules:
            validation['required_verifications'].append(severity_rules['ask_before_marking'])
    ...
    def check_report_closure_requirements(self) -> List[str]:
        ...
        return self.security_context['validation_checks'].get('before_report_closure', {}).get('ask', [])
    
    def check_escalation_needed(self, report_data: Dict) -> Dict[str, Any]:
        """Check if report requires immediate escalation"""
        triggers = self.security_context['escalation_triggers'].get('immediate_escalation', [])
        
        title = report_data.get('attributes', {}).get('title', '').lower()
        vuln_info = report_data.get('attributes', {}).get('vulnerability_information', '').lower()
        ...
        return {
            ...
            "reasons": reasons,
            "questions": self.security_context['escalation_triggers'].get('questions_during_escalation', [])
        }
...
class HackerOneMCPServer(Server):
    def __init__(self):
        ...
        print(f"Credentials configured: {bool(H1_USERNAME and H1_API_TOKEN)}", file=sys.stderr)
        print(f"MDC directives loaded: {self.mdc_directives is not None}", file=sys.stderr)
    ...
    def _load_mdc_directives(self):
        ...
        try:
            ...
            if mdc_path.exists():
                ...
                print(f"MDC directives loaded successfully from {MDC_FILE_PATH}", file=sys.stderr)
            else:
                print(f"MDC file not found at {MDC_FILE_PATH}", file=sys.stderr)
        except Exception as e:
            print(f"Error loading MDC directives: {e}", file=sys.stderr)
    
    def _build_tool_description_with_mdc(self, tool_name: str, base_description: str) -> str:
        ...
        # Add core mission
        core_mission = self.mdc_directives.get('core_directives', {}).get('primary_mission', '')
        if core_mission:
        ...
        decision_framework = self.mdc_directives.get('operational_principles', {}).get('decision_framework', [])
        if decision_framework:
            enhanced_description += f"\n\nDECISION FRAMEWORK: {'; '.join(decision_framework[:2])}"
        
        return enhanced_description
    
    def _add_mdc_guidance_to_response(self, tool_name: str, response_data: Any) -> Dict[str, Any]:
        """Add MDC guidance to tool responses"""
        if not self.mdc_checker:
            return response_data
        
        mdc_guidance = {
            "security_reminder": self.mdc_directives.get('reminder', {}).get('core_philosophy', ''),
            "guidelines": {}
        }
        ...
        if tool_name == "get_new_reports" or tool_name == "check_report":
            ...
            mdc_guidance["researcher_guidelines"] = self.mdc_checker.get_researcher_interaction_guidelines()
...
def analyze_scope(program_data, report_data):
    ...
    if structured_scope:
        ...
        for scope in structured_scope:
            if scope["eligible_for_submission"]:
                ...
                max_sev = f" (Max: {scope['max_severity']})" if scope['max_severity'] else ""
    ...
    if not asset_identifier:
        ...
        for scope in structured_scope:
            ...
            if (asset_identifier == scope_asset_id and 
                asset_type.lower() == scope_asset_type.lower()):
        ...
        if scope_matches:
            in_scope_matches = [m for m in scope_matches if m["scope"]["eligible_for_submission"]]
            if in_scope_matches:
                result += "**✅ IN SCOPE**\n"
                for match in in_scope_matches:
                    scope = match["scope"]
                    bounty_eligible = "💰 Bounty eligible" if scope["eligible_for_bounty"] else "📝 No bounty"
                    max_sev = f" (Max severity: {scope['max_severity']})" if scope['max_severity'] else ""
...
def analyze_duplicates(target_report, all_reports):
    ...
    if "reporter" in target_report.get("relationships", {}):
        ...
        if "attributes" in reporter_data:
            target_reporter_username = reporter_data["attributes"].get('username', '').lower()
    ...
    for report in all_reports:
        ...
        if "structured_scope" in report.get("relationships", {}):
            ...
            if "attributes" in scope_data:
                asset_identifier = scope_data["attributes"].get('asset_identifier', '').lower()
        ...
        if "reporter" in report.get("relationships", {}):
            ...
            if "attributes" in reporter_data:
                reporter_username = reporter_data["attributes"].get('username', '').lower()
        ...
        if target_vuln_info and vuln_info:
            ...
            if len(target_words) > 0 and len(report_words) > 0:
                ...
                similarity_ratio = len(common_words) / max(len(target_words), len(report_words))
    ...
    if exact_matches:
        ...
        for i, match in enumerate(exact_matches, 1):
            report = match['report']
            attrs = report.get('attributes', {})
            result += f"**{i}. Report #{report['id']}** (Similarity: {match['score']}%)\n"
...
def generate_weekly_report(all_reports):
        ...
        if created_at_str:
            try:
                ...
                if created_at >= week_start:
                    ...
                    vuln_info = report['attributes'].get('vulnerability_information', '').lower()
                    
                    compliance_indicators = [
                        'personal data', 'pii', 'phi', 'cardholder data', 'payment',
                        'gdpr', 'hipaa', 'pci', 'sox', 'financial', 'breach',
                        'encryption', 'authentication', 'authorization', 'access control',
                        'audit', 'logging', 'compliance', 'regulation', 'zero-knowledge',
                        'client-side', 'private key', 'vault', 'password', 'argon2', 'curve25519'
                    ]
                    
                    if any(indicator in title or indicator in vuln_info for indicator in compliance_indicators):
                ...
                if report['attributes'].get('state') == 'new' and created_at >= week_start:
    ...
    result += f"- **Cryptographic Key Separation**: All implementations verified secure\n"
    ...
    result += f"- **Reports with Recent Activity**: {len(reports_with_recent_activity)}\n"
    ...
    if closed_reports_this_week:
        ...
        for report in closed_reports_this_week[:10]:  # Show top 10
            ...
            if state == 'resolved':
                result += f"- **Privacy Impact**: Zero-knowledge architecture preserved ✅\n"
        ...
        if len(closed_reports_this_week) > 10:
            result += f"... and {len(closed_reports_this_week) - 10} more closed reports\n\n"
    ...
    if reports_with_recent_activity:
        ...
        for report in reports_with_recent_activity[:5]:  # Show top 5
            ...
            if attrs.get('state') == 'new':
                ...
                result += "Under technical investigation with privacy preservation focus\n"
    ...
    if len(monthly_reports) > 0:
        ...
        current_week_vs_avg = (len(weekly_reports) / avg_weekly_submissions) * 100 if avg_weekly_submissions > 0 else 0
        ...
        privacy_keywords = ['encrypt', 'key', 'password', 'vault', 'client', 'privacy', 'zero-knowledge']
        privacy_related_monthly = sum(1 for r in monthly_reports if any(keyword in r['attributes'].get('title', '').lower() for keyword in privacy_keywords))
        privacy_related_weekly = sum(1 for r in weekly_reports if any(keyword in r['attributes'].get('title', '').lower() for keyword in privacy_keywords))
...
def format_report_details(report_data, mdc_guidance=None):
    ...
    if attrs.get('first_program_activity_at'):
        timeline_items.append(f"- First Program Response: {attrs['first_program_activity_at']}")
    if attrs.get('last_program_activity_at'):
        timeline_items.append(f"- Last Program Activity: {attrs['last_program_activity_at']}")
    if attrs.get('last_reporter_activity_at'):
        timeline_items.append(f"- Last Reporter Activity: {attrs['last_reporter_activity_at']}")
    ...
    if attrs.get('timer_first_program_response_miss_at'):
        timer_info.append(f"Response Due: {attrs['timer_first_program_response_miss_at']}")
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> Sequence[TextContent]:
    ...
    if name not in ["get_new_reports", "get_all_reports", "check_report", "check_scope", "check_duplicate", "make_weekly_report"]:
    ...
    try:
        ...
        if name == "check_report":
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                print(f"Report API response status: {response.status_code}", file=sys.stderr)
            ...
            if server.mdc_checker:
                ...
                escalation_check = server.mdc_checker.check_escalation_needed(report_data)
                if escalation_check["escalate"]:
                    result_text += "### ⚠️ IMMEDIATE ESCALATION REQUIRED\n"
                    result_text += f"**Reasons**: {', '.join(escalation_check['reasons'])}\n"
            ...
            if server.mdc_directives:
                reminder = server.mdc_directives.get('reminder', {}).get('core_philosophy', '')
                if reminder:
                    result_text += f"\n---\n\n💡 **Remember**: {reminder}\n"
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                print(f"Report API response status: {report_response.status_code}", file=sys.stderr)
                ...
                print(f"Programs API response status: {programs_response.status_code}", file=sys.stderr)
            ...
            if server.mdc_checker:
                ...
                mdc_section += "Please verify the following before making a scope determination:\n"
                for check in ["Verify the asset matches program scope exactly",
                            "Check if asset is eligible for bounty",
                            "Review maximum severity allowed for this asset",
                            "Consider any special instructions or limitations"]:
                    mdc_section += f"- [ ] {check}\n"
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                print(f"Target report API response status: {target_response.status_code}", file=sys.stderr)
                ...
                while True:
                    ...
                    print(f"Fetching page {page_number} for duplicate analysis (page size: {page_size})...", file=sys.stderr)
                    ...
                    print(f"API response status for page {page_number}: {response.status_code}", file=sys.stderr)
                    ...
                    print(f"Page {page_number}: Found {page_count} reports for duplicate analysis", file=sys.stderr)
                    
                    if page_count == 0:
                        print(f"Page {page_number} returned 0 reports, stopping pagination", file=sys.stderr)
                        break
                    
                    all_reports.extend(page_data)
                    total_fetched += page_count
                    
                    print(f"Total reports fetched so far for duplicate analysis: {total_fetched}", file=sys.stderr)
                    
                    page_number += 1
                
                print(f"Duplicate analysis pagination complete. Total reports fetched: {len(all_reports)}", file=sys.stderr)
            ...
            if server.mdc_checker:
                ...
                for check in ["Consider similarity score thresholds carefully",
                            "Check if vulnerabilities have same root cause",
                            "Verify if fixes would be identical",
                            "Document reasoning for duplicate determination",
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                while True:
                    ...
                    print(f"Fetching page {page_number} for weekly report (page size: {page_size})...", file=sys.stderr)
                    ...
                    print(f"Page {page_number}: Found {page_count} reports for weekly analysis", file=sys.stderr)
                    ...
                    print(f"Total reports fetched so far for weekly analysis: {total_fetched}", file=sys.stderr)
                    
                    page_number += 1
                
                print(f"Weekly report pagination complete. Total reports fetched: {len(all_reports)}", file=sys.stderr)
            ...
            if server.mdc_checker:
                ...
                mdc_section += "- [ ] Program scope and policies are being consistently applied\n"
        ...
        if name == "get_new_reports":
            ...
            while True:
                ...
                print(f"Fetching page {page_number} (page size: {page_size})...", file=sys.stderr)
                ...
                print(f"Page {page_number}: Found {page_count} reports", file=sys.stderr)
            ...
            print(f"Pagination complete. Total reports fetched: {len(all_reports)}", file=sys.stderr)

        if not all_reports:
            no_reports_message = "No new reports found." if name == "get_new_reports" else "No reports found."
            return [
                TextContent(
                    type="text",
                    text=no_reports_message
                )
            ]
        
        # Format comprehensive report details
        tool_title = "New HackerOne Reports" if name == "get_new_reports" else "All HackerOne Reports"
        ...
        for i, report in enumerate(all_reports, 1):
            attrs = report["attributes"]
            state_info = f"{attrs['main_state']} ({attrs['state']})" if name == "get_all_reports" else attrs['state']
        ...
        return [
            TextContent(
                type="text",
                text="Error: Request to HackerOne API timed out."
            )
        ]
    except Exception as e:
